...
public class JobSubmitterPanel extends JPanel {
    ...
    private JTextField submitterIdField;
    ...
    public JobSubmitterPanel(User currentUser, JobService jobService, CloudLogService cloudLogService) {
        ...
        jobsModel = new DefaultTableModel(
                new Object[]{"Queue Rank", "Job ID", "Assigned Vehicle", "Description", "Duration (hrs)", "Deadline", "Cost", "Interrupts", "Failures", "Status", "Reports"},
                0) {
            @Override
    ...
    private JPanel createSubmitPanel() {
        ...
        int row = 0;
        addLabel(panel, "Job Submitter ID:", gbc, row);
        submitterIdField = addField(panel, gbc, row++);
        submitterIdField.setText(currentUser.getUserId());
        submitterIdField.setEditable(false);
    ...
    private void submitJob() {
        String submitterId = submitterIdField.getText().trim();
        String durationText = durationField.getText().trim();
        String deadlineText = deadlineField.getText().trim();
        String costText = submissionCostField.getText().trim();
        String description = descriptionField.getText().trim();

        if (submitterId.isEmpty() || durationText.isEmpty() || deadlineText.isEmpty() || costText.isEmpty() || description.isEmpty()) {
        ...
        writeAudit(String.format(
                "[%s] ROLE: JOB_SUBMITTER | JOB_SUBMITTER_ID: %s | JOB_ID: %s | SUBMISSION_TIMESTAMP: %s | COST_FOR_SUBMISSION: %s | JOB_DESCRIPTION: %s | STATUS: %s",
    ...
    private void updateStatus() {
        ...
        writeAudit(String.format(
                "[%s] ROLE: JOB_SUBMITTER | JOB_SUBMITTER_ID: %s | JOB_ID: %s | ACTION: UPDATE_STATUS | STATUS: %s",
                LocalDateTime.now().format(TS_FMT),
    ...
    private void addReportLine() {
        ...
        writeAudit(String.format(
                "[%s] ROLE: JOB_SUBMITTER | JOB_SUBMITTER_ID: %s | JOB_ID: %s | ACTION: ADD_REPORT | REPORT_LINE: %s",
                LocalDateTime.now().format(TS_FMT),
                currentUser.getUserId(),
                selected.get().getJobId(),
                safe(reportLine)));
    ...
    private void incrementInterrupts() {
        ...
        writeAudit(String.format(
                "[%s] ROLE: JOB_SUBMITTER | JOB_SUBMITTER_ID: %s | JOB_ID: %s | ACTION: INCREMENT_INTERRUPT",
                LocalDateTime.now().format(TS_FMT),
                currentUser.getUserId(),
                selected.get().getJobId()));
    ...
    private void incrementFailures() {
        ...
        writeAudit(String.format(
                "[%s] ROLE: JOB_SUBMITTER | JOB_SUBMITTER_ID: %s | JOB_ID: %s | ACTION: INCREMENT_FAILURE",
                LocalDateTime.now().format(TS_FMT),
                currentUser.getUserId(),
                selected.get().getJobId()));
    ...
    private void refreshJobsTable() {
        String selectedId = selectedJobIdField != null ? selectedJobIdField.getText().trim() : "";
